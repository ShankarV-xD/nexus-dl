...
android {
    namespace = "com.kira.ytdlp"
    compileSdk = 35

    defaultConfig {
        applicationId = "com.kira.ytdlp"
        minSdk = 24
        targetSdk = 35
        versionCode = 1
        versionName = "1.0.0"
    ...
    signingConfigs {
        create("release") {
            storeFile = file(localProps.getProperty("KEYSTORE_PATH", "nexusdl-release.jks"))
            storePassword = localProps.getProperty("KEYSTORE_PASSWORD", "")
            keyAlias = localProps.getProperty("KEY_ALIAS", "nexusdl")
    ...
    // Produce per-ABI APKs AND a universal APK in one build
    splits {
        abi {
            isEnable = true
            reset()
